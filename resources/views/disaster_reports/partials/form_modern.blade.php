...
<form id="{{ $formId }}" action="{{ $action }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="col-lg-6">
            <div class="card h-100 shadow-sm border-0 rounded-4">
                ...
                <div class="card-body pt-0">
                    <div class="mb-3">
                        <label class="form-label text-muted small fw-bold" for="organization_name">ชื่อหน่วยงาน /
                            สถานศึกษา <span class="text-danger">*</span></label>
                        <input type="text" class="form-control bg-light border-0" id="organization_name"
                            name="organization_name"
                            value="{{ old('organization_name', $report->organization_name ?? '') }}" required
                            placeholder="พิมพ์ชื่อโรงเรียนเพื่อค้นหา" autocomplete="off">
                        <input type="hidden" id="school_id" name="school_id"
                            value="{{ old('school_id', $report->school_id ?? '') }}">
                        <div id="school_suggestions" class="list-group position-absolute"
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label text-muted small fw-bold" for="district_id">อำเภอ <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control bg-light border-0" id="district_display" disabled
                                placeholder="จะถูกเติมอัตโนมัติเมื่อเลือกโรงเรียน">
                            <select class="form-select bg-light border-0 d-none" id="district_id" name="district_id"
                                required>
                                <option value="">เลือกอำเภอ</option>
                                @foreach ($districts as $district)
                                    <option value="{{ $district->id }}" @selected(old('district_id', $report->district_id ?? '') == $district->id)>
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-muted small fw-bold" for="affiliation_id">สังกัด <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control bg-light border-0" id="affiliation_display"
                                disabled placeholder="จะถูกเติมอัตโนมัติเมื่อเลือกโรงเรียน">
                            <select class="form-select bg-light border-0 d-none" id="affiliation_id"
                                name="affiliation_id" required>
                                <option value="">เลือกสังกัด</option>
                                @foreach ($affiliations as $affiliation)
                                    <option value="{{ $affiliation->id }}" @selected(old('affiliation_id', $report->affiliation_id ?? '') == $affiliation->id)>
                    ...
                    <div class="mb-3">
                        <label class="form-label text-muted small fw-bold" for="teaching_status">สถานะการเรียนการสอน
                            <span class="text-danger">*</span></label>
                        <select class="form-select bg-light border-0" id="teaching_status" name="teaching_status"
                            required>
            ...
            <div class="card h-100 shadow-sm border-0 rounded-4">
                <div class="card-header bg-white py-3 border-bottom-0 rounded-top-4">
                    <h5 class="mb-0 fw-bold text-primary"><i
                            class="bi bi-exclamation-triangle-fill me-2"></i>ข้อมูลเหตุการณ์</h5>
                </div>
                <div class="card-body pt-0">
                    {{-- reported_at removed as per request --}}

                    <div class="mb-3">
                        <label class="form-label text-muted small fw-bold" for="disaster_type_select">ประเภทภัยพิบัติ
                            <span class="text-danger">*</span></label>
                        @php
                        ...
                        <select class="form-select bg-light border-0" id="disaster_type_select" required>
                            <option value="" disabled {{ empty($currentType) ? 'selected' : '' }}>
                                เลือกประเภทภัยพิบัติ</option>
                        ...
                        <input type="text" class="form-control mt-2 bg-light border-0" id="disaster_type_other"
                            placeholder="ระบุประเภทภัยพิบัติ" value="{{ $isCustom ? $currentType : '' }}"
                            style="{{ $isCustom ? '' : 'display: none;' }}">
                        <x-input-error name="disaster_type" />
                    </div>

                    <div class="mb-3">
                        <label class="form-label text-muted small fw-bold" for="current_status_select">สถานการณ์ปัจจุบัน
                            <span class="text-danger">*</span></label>
                        @php
                        ...
                        <select class="form-select bg-light border-0" id="current_status_select" required>
                            <option value="" disabled {{ empty($currentStatus) ? 'selected' : '' }}>
                                เลือกสถานการณ์</option>
                            @foreach ($statusTypes as $status)
                                <option value="{{ $status }}"
                                    {{ $currentStatus == $status ? 'selected' : '' }}>{{ $status }}</option>
                            @endforeach
                            <option value="other" {{ $isCustomStatus ? 'selected' : '' }}>อื่นๆ (ระบุ)</option>
                        </select>
                        <input type="text" class="form-control mt-2 bg-light border-0" id="current_status_other"
                            placeholder="ระบุสถานการณ์" value="{{ $isCustomStatus ? $currentStatus : '' }}"
                            style="{{ $isCustomStatus ? '' : 'display: none;' }}">
        ...
        <div class="card-body pt-0">
            <div class="row g-4">
                ...
                <div class="col-md-6 border-end-md">
                    <h6 class="fw-bold text-secondary mb-3 bg-light p-2 rounded"><i
                            class="bi bi-backpack me-2"></i>นักเรียน</h6>
                    <div class="row g-3">
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">ได้รับผลกระทบ</label>
                            <input type="number" min="0"
                                class="form-control text-center fw-bold text-primary" id="affected_students"
                                name="affected_students"
                                value="{{ old('affected_students', $report->affected_students ?? 0) }}" required>
                        </div>
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">บาดเจ็บ</label>
                            <input type="number" min="0"
                                class="form-control text-center fw-bold text-warning" id="injured_students"
                                name="injured_students"
                                value="{{ old('injured_students', $report->injured_students ?? 0) }}" required>
                        </div>
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">เสียชีวิต</label>
                            <input type="number" min="0" class="form-control text-center fw-bold text-danger"
                                id="dead_students" name="dead_students"
                                value="{{ old('dead_students', $report->dead_students ?? 0) }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label text-muted small fw-bold">รายชื่อผู้เสียชีวิต (ถ้ามี)</label>
                            <textarea class="form-control bg-light border-0" rows="2" id="dead_students_list" name="dead_students_list"
                                placeholder="ระบุชื่อ-นามสกุล">{{ old('dead_students_list', $report->dead_students_list ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <h6 class="fw-bold text-secondary mb-3 bg-light p-2 rounded"><i
                            class="bi bi-person-badge me-2"></i>บุคลากร</h6>
                    <div class="row g-3">
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">ได้รับผลกระทบ</label>
                            <input type="number" min="0"
                                class="form-control text-center fw-bold text-primary" id="affected_staff"
                                name="affected_staff"
                                value="{{ old('affected_staff', $report->affected_staff ?? 0) }}" required>
                        </div>
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">บาดเจ็บ</label>
                            <input type="number" min="0"
                                class="form-control text-center fw-bold text-warning" id="injured_staff"
                                name="injured_staff" value="{{ old('injured_staff', $report->injured_staff ?? 0) }}"
                                required>
                        </div>
                        <div class="col-4">
                            <label class="form-label text-muted small fw-bold">เสียชีวิต</label>
                            <input type="number" min="0" class="form-control text-center fw-bold text-danger"
                                id="dead_staff" name="dead_staff"
                                value="{{ old('dead_staff', $report->dead_staff ?? 0) }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label text-muted small fw-bold">รายชื่อผู้เสียชีวิต (ถ้ามี)</label>
                            <textarea class="form-control bg-light border-0" rows="2" id="dead_staff_list" name="dead_staff_list"
                                placeholder="ระบุชื่อ-นามสกุล">{{ old('dead_staff_list', $report->dead_staff_list ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="card shadow-sm border-0 mb-4 rounded-4">
        <div class="card-header bg-white py-3 border-bottom-0 rounded-top-4">
            <h5 class="mb-0 fw-bold text-warning"><i class="bi bi-currency-exchange me-2"></i>ประมาณการความเสียหาย
            </h5>
        </div>
        <div class="card-body pt-0">
            <div class="row g-3 mb-3">
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold" for="damage_building">อาคารสถานที่
                        (บาท)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-0">฿</span>
                        <input type="number" step="0.01" min="0" class="form-control bg-light border-0"
                            id="damage_building" name="damage_building"
                            value="{{ old('damage_building', number_format((float) ($report->damage_building ?? 0), 2, '.', '')) }}"
                            required>
                    </div>
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold" for="damage_equipment">ครุภัณฑ์การศึกษา
                        (บาท)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-0">฿</span>
                        <input type="number" step="0.01" min="0" class="form-control bg-light border-0"
                            id="damage_equipment" name="damage_equipment"
                            value="{{ old('damage_equipment', number_format((float) ($report->damage_equipment ?? 0), 2, '.', '')) }}"
                            required>
                    </div>
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold" for="damage_material">วัสดุอุปกรณ์
                        (บาท)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-0">฿</span>
                        <input type="number" step="0.01" min="0" class="form-control bg-light border-0"
                            id="damage_material" name="damage_material"
                            value="{{ old('damage_material', number_format((float) ($report->damage_material ?? 0), 2, '.', '')) }}"
                            required>
                    </div>
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold" for="damage_total_request">ขอรับการสนับสนุน
                        (บาท)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-warning border-0 text-dark">฿</span>
                        <input type="number" step="0.01" min="0"
                            class="form-control bg-warning bg-opacity-10 border-0 fw-bold" id="damage_total_request"
                            name="damage_total_request"
                            value="{{ old('damage_total_request', number_format((float) ($report->damage_total_request ?? 0), 2, '.', '')) }}"
                            required>
                    </div>
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label text-muted small fw-bold"
                    for="assistance_received">การให้ความช่วยเหลือเยียวยาเบื้องต้น (ถ้ามี)</label>
                <textarea class="form-control bg-light border-0" rows="3" id="assistance_received" name="assistance_received"
                    placeholder="ระบุรายละเอียดการช่วยเหลือที่ได้รับแล้ว">{{ old('assistance_received', $report->assistance_received ?? '') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0 mb-4 rounded-4">
        ...
        <div class="card-body pt-0">
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label text-muted small fw-bold" for="contact_name">ชื่อ-นามสกุล <span
                            class="text-danger">*</span></label>
                    <input type="text" class="form-control bg-light border-0" id="contact_name"
                        name="contact_name"
                        value="{{ old('contact_name', $report->contact_name ?? (auth()->user()->name ?? '')) }}"
                        required>
                </div>
                <div class="col-md-4">
                    <label class="form-label text-muted small fw-bold" for="contact_position">ตำแหน่ง <span
                            class="text-danger">*</span></label>
                    <input type="text" class="form-control bg-light border-0" id="contact_position"
                        name="contact_position"
                        value="{{ old('contact_position', $report->contact_position ?? '') }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label text-muted small fw-bold" for="contact_phone">เบอร์โทรศัพท์ <span
                            class="text-danger">*</span></label>
                    <input type="tel" class="form-control bg-light border-0" id="contact_phone"
                        name="contact_phone"
                        value="{{ old('contact_phone', $report->contact_phone ?? (auth()->user()->tel ?? '')) }}"
        ...
        <div class="card-body pt-0">
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.0000001" class="form-control bg-light border-0" id="latitude"
                        name="latitude" value="{{ old('latitude', $report->latitude ?? '') }}" min="-90"
                        max="90" placeholder="เช่น 6.54321">
                </div>
                <div class="col-md-6">
                    <label class="form-label text-muted small fw-bold" for="longitude">ลองจิจูด</label>
                    <input type="number" step="0.0000001" class="form-control bg-light border-0" id="longitude"
                        name="longitude" value="{{ old('longitude', $report->longitude ?? '') }}" min="-180"
                        max="180" placeholder="เช่น 101.12345">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="text" class="form-control bg-light border-0" id="address_search"
                        placeholder="กรอกที่อยู่หรือชื่อสถานที่เพื่อค้นหาพิกัด">
            ...
            <!-- Map Container -->
            <div id="map" style="height: 400px; width: 100%; border-radius: 16px; z-index: 1;"
            ...
            <div class="card shadow-sm border-0 rounded-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <small class="text-muted"><i class="bi bi-info-circle me-1"></i>รองรับ: JPG, PNG (สูงสุด
                            5MB)</small>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="currentImagePreview">
                                ...
                                <p class="text-muted small mt-2"><i
                                        class="bi bi-info-circle me-1"></i>อัปโหลดรูปใหม่เพื่อแทนที่รูปเดิม</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <div id="imagePreview" style="display: none;" class="mt-3">
                        ...
                        <br>
                        <img id="imagePreviewImg" src="" class="img-fluid rounded shadow-sm"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            ...
            <div class="modal-body text-center py-4">
                ...
                <p class="mb-0 text-muted">ตรวจสอบความถูกต้องของข้อมูลก่อนยืนยันการบันทึก<br>หากข้อมูลถูกต้องให้กด
                    "ยืนยัน" เพื่อดำเนินการต่อ</p>
            </div>
            <div class="modal-footer border-top-0 justify-content-center pb-4">
                ...
                <button type="button" class="btn btn-primary px-4 rounded-pill"
                    data-confirm>{{ $submitLabel }}</button>
            </div>
        </div>
    </div>
</div>
                            ...
                            .then(schools => {
                                if (schools.length === 0) {
                                    suggestionsBox.innerHTML =
                                        '<div class="list-group-item text-muted">ไม่พบโรงเรียน</div>';
                                ...
                                suggestionsBox.querySelectorAll('button').forEach(btn => {
                                    btn.addEventListener('click', function() {
                                        const school = JSON.parse(this.dataset
                                            .school);
                                        selectSchool(school);
                                    });
                                });
